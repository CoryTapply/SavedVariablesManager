# Handoff: SavedVariables Comparator — compare screen

## Overview

The WoW SavedVariables Comparator diffs two accounts' addon SavedVariables files
(WeakAuras.lua, Plater.lua, …) and can copy one account's file over another's. This handoff
covers **one screen, redesigned end to end**: selecting what to compare, and reading the
comparison.

Two changes against today's app:

1. **Selection is a single command bar**, not two stacked "Account A" / "Account B" form
   cards. Addon, account A, account B, Compare and Copy A → B sit in one horizontal toolbar.
2. **Results are a master–detail split**, not a long list of expandable per-aura rows: a
   scrollable list of changed auras on the left, the selected aura's full field-level diff
   on the right.
...
Selection and results are one flow in one window, not two screens. Pressing **Compare**
collapses the command bar into a one-line context strip at the top and opens the split
underneath it. **Change selection** expands the bar again.

## About the design files

The file in this bundle is a **design reference written in HTML** — a prototype of the
intended look and behavior, not production code to paste in. Recreate it in the app's
existing environment (its current framework, component layer and styling approach),
following the patterns already established there. The prototype uses a small ad-hoc `.zp-*`
class layer purely so it could be a single file; do not port that layer — map each class to
the equivalent component or token usage in the real codebase.

The CSS in `design_system/` **is** production-usable: those token files are the real Zerpy
tokens, and every value quoted below comes from them.

## Fidelity

**High fidelity.** Colors, typography, spacing, radii, elevation and motion are final.
Recreate the layout to the measurements given, and use the CSS variables rather than the
literal hex values wherever possible.

Aura names, byte sizes and field paths are representative sample data. Copy strings in
...
One window with two states — **idle** (selection) and **compared** (results) — plus three
overlays (settings dialog, copy toast, reveal-in-explorer dialog).

### Shell — both states

- Window ground: `background: var(--zp-ground)` over `background-color: var(--zp-bg)`.
- Starfield: an absolutely positioned `inset: 0` layer, `pointer-events: none`,
...
- No app title bar or chrome strip inside the frame — content starts at the top edge.
- The prototype frame is 1180px wide and `min-height: 720px`; the real window is resizable
  and everything below reflows. The results split keeps a fixed 360px left list.

---

### 1. Idle state — selection

**Purpose:** choose an addon and two accounts, then compare or copy.

**Layout:** one column, `padding: var(--zp-space-7) var(--zp-space-7) var(--zp-space-8)`
(32 / 32 / 40px), `display: flex; flex-direction: column; gap: var(--zp-space-6)` (24px).
Three blocks: header row, command bar, results placeholder.
...
- Title **"Compare SavedVariables files"** — `font: var(--zp-text-h1)` (500 30px/1.16
  Instrument Sans), `color: var(--zp-text)`.
...
Right — `display: flex; gap: var(--zp-space-3)`:
- Flavor pill "Classic Era ▾" — `height: var(--zp-control-h-sm)` (30px),
...
**1.2 Command bar** — the core change. One panel: `background: var(--zp-surface-panel)`,
`1px solid var(--zp-line)`, `border-radius: var(--zp-radius)` (4px),
`backdrop-filter: blur(var(--zp-blur-md))`, `box-shadow: var(--zp-elev-panel)`,
`padding: var(--zp-space-4)` (16px),
`display: flex; align-items: center; gap: var(--zp-space-4); flex-wrap: wrap`.

| # | Element | Spec |
| --- | --- | --- |
| 1 | Addon select — "◆ WeakAuras" | 190px wide, `height: var(--zp-control-h)` (38px), `background: var(--zp-surface-field)`, `1px solid var(--zp-line)`, `var(--zp-radius)`, `padding: 0 var(--zp-space-4)`, label left / `▾` right in `var(--zp-text-4)`. No label above it. |
| 2 | Vertical divider | 1px × 28px, `background: var(--zp-line)` |
| 3 | Account A group | `flex-direction: column; gap: 4px`. Label "Account A" — `font: var(--zp-text-label)` at 10px, uppercase, `letter-spacing: var(--zp-tracking-micro)`, `color: var(--zp-text-3)`. Select 220px, same spec as (1); shows the account's **name** with its raw id beside it in `var(--zp-text-4)`, or the bare id if unnamed. |
| 4 | Swap glyph | `⇄`, 18px, `color: var(--zp-text-4)`, `padding-top: 14px` to sit on the selects' centerline. Swaps A and B. |
| 5 | Account B group | Same as (3). |
| 6 | Spacer | `flex: 1` |
...
**1.3 Results placeholder** — flat panel (`background: var(--zp-surface-1)`,
`1px solid var(--zp-line)`, `var(--zp-radius)`, **no blur** — it sits inside the page, not
as a second glass layer), `padding: var(--zp-space-8)`, centered text,
`font: var(--zp-text-sm)`, `color: var(--zp-text-4)`, copy: **"Press Compare — this area
becomes the comparison"**. It holds the space the results will take, so nothing jumps.
...
- "Classic Era" · "◆ WeakAuras"
- A pill: an `A` badge (success variant, 16px tall, `padding: 0 5px`) + the account label
  + "24.8 MB" in `var(--zp-text-sm)`/`var(--zp-text-4)`
- `⇄` in `var(--zp-text-4)`
- B pill: a `B` badge (warning variant) + label + "24.1 MB"
- spacer, then a small ghost button **"Change selection"**
  (`height: var(--zp-control-h-sm)`, `padding: 0 var(--zp-space-4)`, 11.5px) → idle state.
...
- `● 8 added` (success dot #5ec48a) · `● 12 removed` (danger #e8636b) ·
  `● 116 changed` (warning #e8a44c). Dot 7px, `border-radius: 999px`.
- Pill base: `height: var(--zp-control-h-sm); padding: 0 var(--zp-space-4); font: var(--zp-text-sm); color: var(--zp-text-2)`.
- Active: `background: var(--zp-accent-tint); border-color: var(--zp-line-accent); color: var(--zp-accent-200)`.
- Then "3417 unchanged, hidden" — `var(--zp-text-sm)`/`var(--zp-text-4)`, not a pill, not
  clickable; unchanged entries are never listed.
- While a filter is active, a small ghost **"Clear filter"** button appears at the end.

A full-width 1px `var(--zp-line)` divider closes the strip.

**2.2 The split** — `display: flex; align-items: stretch`, fills the remaining window height
(560px in the prototype).

**Left: aura list** — `width: 360px; flex: none; border-right: 1px solid var(--zp-line); overflow-y: auto`.
- List toolbar: `padding: var(--zp-space-3) var(--zp-space-4)`,
  `border-bottom: 1px solid var(--zp-line)`. Holds a single full-width filter field
  (`flex: 1`, `height: var(--zp-control-h-sm)`, `background: var(--zp-surface-field)`,
  `1px solid var(--zp-line)`, `padding: 0 10px`, placeholder "⌕ Filter…" in
  `var(--zp-text-4)`). **No kind selector here** — the stat pills above are the kind filter,
  and duplicating them in the toolbar was cut deliberately.
- Row: `justify-content: space-between; padding: 10px var(--zp-space-4); border-bottom: 1px solid var(--zp-line); border-left: 2px solid transparent; cursor: pointer`.
  ...
  for changed auras the **field count as a bare number** (warning variant), for added or
  removed the word "added" / "removed" (success / danger variant).
- Selected row: `border-left: 2px solid var(--zp-accent); background: var(--zp-accent-tint)`;
  the name becomes `font: var(--zp-text-label); color: var(--zp-text)`.
- Hover (unselected): `background: var(--zp-surface-2)`,
  `border-left-color: var(--zp-line-accent)`.
- Footer row: "… 131 more" in `var(--zp-text-sm)`/`var(--zp-text-4)`,
  `padding: 10px var(--zp-space-4)`. The real list is long — virtualize it; the count follows
  the active filter (all 131 / added 7 / removed 11 / changed 113).
...
- One block per changed field, `gap: var(--zp-space-2)`:
  - Header: a "changed" warning badge + the field path in `var(--zp-text-data)`,
    `color: var(--zp-text-3)`, written with `›` separators — `load › class`,
    `actions › init › custom`, `trigger › 1 › spellName`.
  - **Scalar values** inline: before in `var(--zp-danger)`, `→` in `var(--zp-text-4)`,
    after in `var(--zp-success)`, all `font: var(--zp-text-data)`, `gap: var(--zp-space-3)`,
    `padding-left: 4px`.
  - **Multi-line / code values** as a two-column diff grid: `1px solid var(--zp-line)`,
    `var(--zp-radius)`, `overflow: hidden`. Header strip — `padding: 6px var(--zp-space-3)`,
    `background: var(--zp-surface-1)`, `border-bottom: 1px solid var(--zp-line)`,
    `font: var(--zp-text-micro)`, uppercase, `letter-spacing: var(--zp-tracking-micro)`,
    `color: var(--zp-text-4)`: "Before → After" left, line counts right (`+2` in
    `var(--zp-success)`, `-1` in `var(--zp-danger)`). Body —
    `display: grid; grid-template-columns: 1fr 1fr`; each cell `padding: 8px var(--zp-space-3)`,
    `font: var(--zp-text-data)`, `color: var(--zp-text-2)`, `white-space: pre`; left cell
    `background: var(--zp-danger-tint)`, right cell `background: var(--zp-success-tint); border-left: 1px solid var(--zp-line)`.
...
- Pinned at the bottom of the pane (after a `flex: 1` spacer), a flat panel footnote,
  `padding: var(--zp-space-3) var(--zp-space-4)`: "Other saved data outside of auras also
  differs (not shown above): only in A: `migrationVersion`." — key names in
  `var(--zp-text-data)`.
...
`background: rgba(6,6,14,.62)`, `backdrop-filter: blur(6px)`, content centered.
...
- Two path groups (`gap: 8px`): an uppercase `var(--zp-text-label)` label; a row with a
  read-only path field (`flex: 1`, `height: var(--zp-control-h)`,
  ...
  `overflow: hidden`) and a ghost **"Browse…"** button; then a one-line hint.
...
- **Accounts** group (third): label "Accounts" in `var(--zp-text-label)`, uppercase, then a
  bordered list (`1px solid var(--zp-line)`, `var(--zp-radius)`,
  `background: var(--zp-surface-1)`, `max-height: 176px`, scrolls past four accounts) with
  one row per detected account:
  `display: flex; align-items: center; gap: var(--zp-space-2); padding: 5px var(--zp-space-2) 5px 4px; border-bottom: 1px solid var(--zp-line)`.
  Order is star, name, id — the editable thing sits where the eye lands.
  - Favorite toggle — 28×28 borderless button, `★` when on (`var(--zp-accent-300)`), `☆`
    when off (`var(--zp-text-4)`), 15px. Tooltip "Sort to the top".
  - Name input — `flex: 1`, `height: var(--zp-control-h-sm)`,
    `background: var(--zp-surface-field)`, `1px solid var(--zp-line)`, `padding: 0 10px`,
    `font: var(--zp-text-sm)`, `color: var(--zp-text)`, placeholder "Add a name". Focus takes
    the standard treatment (`--zp-focus-line` + `--zp-focus-shadow`).
  - Account id, right-aligned, `var(--zp-font-data)` 11.5px, `var(--zp-text-4)` — always
    shown, so which folder a name maps to is never ambiguous.
...
- Footer row, `justify-content: space-between`: "3 flavors detected · 4 accounts" in
  `var(--zp-text-sm)`/`var(--zp-text-4)`; right side ghost **Cancel** + primary **Save**.

**Behavior.** Names and favorites are **app-local metadata keyed by account id** — no folder
is renamed or moved. A named account shows its name as the primary label everywhere an
account is picked or displayed (the A/B selects, the context strip) with the id beside it in
`var(--zp-text-4)`; an unnamed account falls back to its raw id. Favorites sort to the top of
every account list, favorites first then the existing order. Persist
`accountMeta: { [accountId]: { name, favorite } }` alongside the folder settings.

### 4. Copy toast

Fires on **Copy A → B**. Stack: `position: absolute; right: var(--zp-space-6); bottom: var(--zp-space-6); z-index: 40`,
column, `gap: var(--zp-space-3)`.
...
- Full destination path in `var(--zp-text-data)` at 11.5px, `var(--zp-text-3)`,
...
- Auto-dismiss after 8s; re-firing resets the timer. Clicking the body reveals the file in
  the OS file manager.
- Past tense and specific, per the Zerpy voice — never "Success!".

### 5. Reveal-in-explorer dialog

Exists in the prototype only to demonstrate the toast's click target. In the real app,
...
| Trigger | Result |
| --- | --- |
| **Compare** | Command bar collapses to the context strip; results open below it, in the same window. |
| **Change selection** | Back to idle. The stat filter resets to "All"; addon/flavor/A/B are preserved. |
| **Stat pill click** | Toggles that kind as the list filter; clicking the active pill returns to "All". The "… N more" count and the visible list follow. |
...
| **Copy A → B** | Back up the target, copy, then show the toast. |
...
- Context strip entering: `zp-collapse-in` — 180ms, from `opacity: 0; translateY(-6px)`.
  Applied to each of the strip's rows.
- Results split entering: `zp-open-below` — 280ms with a **100ms delay**, from
  `opacity: 0; translateY(10px)`. The stagger is what makes it read as the bar collapsing and
  results opening underneath, rather than a screen swap.
...
- No file for the selected addon on one side: **Compare** disables and the affected account
  reads "no WeakAuras file yet" at `opacity: .55`.
- No differences at all: show the context strip with all three counts at 0 and, in place of
  the split, a flat panel stating the two files are identical. (Not drawn — needs a copy
  decision.)
...
Per aura the results need: `id`, `name`, `group` (parent name or empty),
`kind` ('added' | 'removed' | 'changed'), and `fields[]` of
...
All from Zerpy. Use the CSS variables; literals are listed for reference only.
...
`--zp-elev-panel` with `blur(14px)` · overlays `--zp-elev-overlay` with `blur(22px)`; fields
blur 8px. A panel inside a panel is flat.

**Focus** — one treatment everywhere: the control's own border goes to `--zp-focus-line` and
it picks up `--zp-focus-shadow` (1px accent edge + accent glow). Nothing outside the control,
no layout shift. **Disabled** is 45% opacity with the glow removed, never a grey fill.

## Assets

None. No images, icons or illustrations. Every glyph is a Unicode character set in the UI or
mono font at 15px or below: `⚙ ⇄ ⌕ ✕ → ▾ ◆ ● ★ ☆`. Identity and state are carried by 2px left
rails, 7px colored dots and colored badges — Zerpy's substitute for an icon set. If the app
already ships an icon library, keep using it at 1.5px stroke; otherwise do not add one.

## Files

```
Compare Screen.dc.html   the prototype — interactive: Compare, Change selection,
                         stat filters, list selection, Copy A → B toast, settings dialog
...
Open `Compare Screen.dc.html` in a browser and click through it — the state transitions and
the animation stagger are easier to feel than to read.
